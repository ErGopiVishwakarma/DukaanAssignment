...
const Navbar = () => {
  return (
    <div className=' w-56  h-[100vh] flex px-[10px] flex-col bg-[#1E2640] py-4 sticky top-0' style={{fontFamily: "'Inter', sans-serif"}}>
      {/* top section  */}
      <div className='flex justify-between items-center w-full '>
        ...
        <div className=' w-[108px] flex flex-col'>
          ...
          <a href='#'><p className=' text-[13px] opacity-80 text-[#FFFFFF]'>Visit store</p></a>
        </div>
        </div>
      ...
      <div className=' w-full  mt-6 h-full '>
        {
          arr?.map((el) => (
            <div className=' flex pt-2 px-4 text-sm gap-3 rounded opacity-80 font-medium h-9'>
              <img src={el.icon} className=' h-5 w-5' />
              <p className='text-[#FFF] leading-5'>{el.title}</p>
            </div>
          ))
        }
      ...
      {/* bottom section  */}
      <div className='flex gap-3 items-center bottom-0 py-[6px] px-3 bg-[#353C53] rounded relative w-full  '>
